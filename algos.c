...
averageStats print_policy_stat(Queue * processes)
{
	...
	while(process_count < processes->size) {
		...
		float arrival_time = process->arrival_time;
		float run_time = process->runtime;
		float response_time = process->startTime - arrival_time;
		float turnaround = process->endTime - process->arrival_time + 1;
